...
class TempCalc():
    ...
    def heat_tranfer(self, dk, l, v, fl, fe, Kalf, coeff = 0.8):
        Kl = 1+(dk/l)**0.67 
        alfa_duct = Kalf * v**coeff * dk**(-0.2) * Kl * np.sqrt(fe/fl) #0.8

        return alfa_duct
    

def calc(nk, dk, Ds, coeff):
    ...
    alfa_duct = T.heat_tranfer(par.Dk, par.l, Vs, fl, fe, par.k_alf, coeff)
