...
function maximize_acquisition(maximizer::SampleOptAM, acquisition::AcquisitionFunction, problem::BossProblem, options::BossOptions)
    sampler = maximizer.sampler
    opt = maximizer.optimizer
    
    xs, vals = maximize_acquisition(sampler, acquisition, problem, options; return_all=true)

    xs_score = sortperm(vals) |> reverse
    starts = xs[:,xs_score[1:opt.multistart]]

    x, val = maximize_acquisition(set_starts(opt, starts), acquisition, problem, options)
    return x, val
end
