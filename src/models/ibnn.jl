...
function sample_params(model::IBNN)
    return sample_params(model.priors)
end
function sample_params(priors::IBNNPriors)    
    ω = isnothing(priors.omega_priors) ? nothing : rand.(priors.omega_priors)
    λ = hcat(rand.(priors.lengthscale_priors)...)
    α = rand.(priors.amplitude_priors)
    noise_std = rand.(priors.noise_std_priors)
...
function devectorize(params::IBNNParams{Nothing}, p::AbstractVector{<:Real})
    λ_len, α_len, n_len = param_lengths(params)
    λ_shape = size(params.λ)

    λ = reshape(p[1:λ_len], λ_shape)
    α = p[λ_len+1:λ_len+α_len]
    noise_std = p[λ_len+α_len+1:end]

    return IBNNParams(nothing, λ, α, noise_std)
end
function devectorize(params::IBNNParams{<:AbstractVector{<:Real}}, p::AbstractVector{<:Real})
    ω_len, λ_len, α_len, n_len = param_lengths(params)
    λ_shape = size(params.λ)

    ω = p[1:ω_len]
    λ = reshape(p[ω_len+1:ω_len+λ_len], λ_shape)
    α = p[ω_len+λ_len+1:ω_len+λ_len+α_len]
    noise_std = p[ω_len+λ_len+α_len+1:end]

    return IBNNParams(ω, λ, α, noise_std)
end

bijector(::IBNNParams) = InvSoftplus()
